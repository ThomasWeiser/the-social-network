var Elm = Elm || { Native: {} };
Elm.Ads = Elm.Ads || {};
Elm.Ads.make = function (_elm) {
   "use strict";
   _elm.Ads = _elm.Ads || {};
   if (_elm.Ads.values)
   return _elm.Ads.values;
   var _op = {},
   _N = Elm.Native,
...
Elm.Story.make = function (_elm) {
   ...
   if (_elm.Story.values)
   return _elm.Story.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var requestLikeState = F3(function (user,
   story,
   goal) {
      return function () {
         var toAction = function (result) {
            return $Task.succeed(SetLike(function () {
               switch (result.ctor)
               {case "Err":
                  return $Basics.not(goal);
                  case "Ok": return goal;}
               _U.badCase($moduleName,
               "between lines 92 and 97");
            }()));
         };
         var urlChunk = goal ? "like" : "unlike";
         var postLike = A3($Http.post,
   ...
   story) {
      return function () {
         var eventHandler = story.like.pending ? _L.fromArray([]) : _L.fromArray([A2($Html$Events.onClick,
         dispatcher,
         SetLike($Basics.not(story.like.state)))]);
         var likeButton = A2($Html.a,
         A2($List._op["::"],
         $Html$Attributes.href("#"),
         eventHandler),
         _L.fromArray([$Html.text(story.like.state ? "unlike" : "like")]));
         return A2($Html.div,
         _L.fromArray([myStyle]),
         _L.fromArray([A2($Html.p,
                      _L.fromArray([]),
                      _L.fromArray([$User.toLink(story.author)]))
                      ,A2($Html.p,
                      _L.fromArray([]),
                      _L.fromArray([$Html.text(story.message)]))
                      ,A2($Html.p,
                      _L.fromArray([]),
                      _L.fromArray([likeButton]))]));
      }();
   });
   var RequestLike = function (a) {
      return {ctor: "RequestLike"
             ,_0: a};
   };
   var likeStateDecoder = function () {
      var toLikeState = function (bool) {
         return {_: {}
                ,pending: false
                ,state: bool};
      };
   ...
   "like",
   likeStateDecoder));
   _elm.Story.values = {_op: _op
                       ,Model: Model
                       ,LikeState: LikeState
                       ,decoder: decoder
                       ,likeStateDecoder: likeStateDecoder
                       ,RequestLike: RequestLike
                       ,SetLike: SetLike
                       ,update: update
                       ,requestLikeState: requestLikeState
                       ,view: view
                       ,myStyle: myStyle};
   return _elm.Story.values;
};
...
Elm.TheSocialNetwork = Elm.TheSocialNetwork || {};
Elm.TheSocialNetwork.make = function (_elm) {
   "use strict";
   _elm.TheSocialNetwork = _elm.TheSocialNetwork || {};
   if (_elm.TheSocialNetwork.values)
   return _elm.TheSocialNetwork.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "TheSocialNetwork",
   $Ads = Elm.Ads.make(_elm),
   $Basics = Elm.Basics.make(_elm),
